...
return |> filter(date < split_date) %>% select(return) -> train_data
return |> filter(date >= split_date) %>% select(return) -> test_data
...
model_prediction <- function(test_set, whole_data, model){
  # test_set should only have one olumn of predict variable
  # whole_data should have one column 'date' and predict variable
  t <- nrow(test_set)
  Tt <- nrow(whole_data)
  t_start <- Tt - t + 1
  y <- whole_data %>% select(-date)
  predictions <- vector(length = t)
  
  for (i in 1:t){
    n <- t_start - 1 + i
    test_set = whole_data[(n-22):n,]
    test_scaled <- test_set %>% max_min_scale
    x_test = data_prep(test_scaled)$x
  ...
  pred_df <- data.frame('date' = whole_data$date[t_start:Tt] %>% as_date(),
                       'pred' = predictions,
                       'actual' = y[t_start:Tt,])
  pred_df
}
...
model_prediction(test_data, return, lstm_model_new) -> df
```
